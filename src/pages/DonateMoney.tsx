...
export default function DonateMoneyPage() {
  ...
  const validate = () => {
    ...
    if (!amount || parseInt(amount) < 1) e.amount = 'Enter valid amount';
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {presets.map((p) => (
                  <Button type="button" key={p} variant={amount === String(p) ? 'default' : 'outline'} size="sm" onClick={() => setAmount(String(p))}>₹{p}</Button>
                ))}
              </div>
